...
export function middleware(req: NextRequest) {
  const cookies = req.cookies.getAll();  // Get all cookies
  console.log("Cookies received:", cookies);  // Log all cookies to see what's available
  const token = req.cookies.get("jwt")?.value;
  const url = req.nextUrl.clone();
  console.log("Token in middleware:", token);  // Check the token
